...
describe('SAN_WITHDRAWAL_TRANSACTIONS_INTRADAY', () => {
  const expected = {
    date: 'string',
    value: 'number'
  }

  const response = san.SAN_WITHDRAWAL_TRANSACTIONS_INTRADAY(ethereumSlug, from, to)
  const headers = response[0]
  const results = response[1]

  testFieldTypes(results, expected)
  testHandlesNullData('fetchGetMetric', san.SAN_WITHDRAWAL_TRANSACTIONS_INTRADAY, ethereumSlug, from, to)
  ...
  it('returns a record per every day', () => {
    const results = san.SAN_WITHDRAWAL_TRANSACTIONS_INTRADAY(ethereumSlug, from, to)
