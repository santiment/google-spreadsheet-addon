...
describe('SAN_FUNDING_RATE_USDT_MULTIPLE_SLUGS', () => {
  const expected = {
    date: 'string',
    valueSan: 'number',
    valueEth: 'number'
  ...
  const response = san.SAN_FUNDING_RATE_USDT_MULTIPLE_SLUGS(slugsList, from, to)
  const headers = response[0]
  const results = response[1]

  testFieldTypes(results, expected)
  testHandlesNullData('fetchGetMetricMultipleSlugs', san.SAN_FUNDING_RATE_USDT_MULTIPLE_SLUGS, slugsList, from, to)
