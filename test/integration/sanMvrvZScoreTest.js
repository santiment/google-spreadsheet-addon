...
describe('SAN_MVRV_Z_SCORE', () => {
  const expected = {
    date: 'string',
    value: 'number'
  }

  const response = san.SAN_MVRV_Z_SCORE(slug, from, to)
  const headers = response[0]
  const results = response[1]

  testFieldTypes(results, expected)
  testHandlesNullData('fetchGetMetric', san.SAN_MVRV_Z_SCORE, slug, from, to)
  ...
  it('returns a record per every day', () => {
    const results = san.SAN_MVRV_Z_SCORE(slug, from, to)
