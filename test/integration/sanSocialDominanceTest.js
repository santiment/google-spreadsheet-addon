...
describe('SAN_SOCIAL_DOMINANCE', () => {
  ...
  const response = san.SAN_SOCIAL_DOMINANCE(slug, from, to)
  const headers = response[0]
  const results = response[1]

  testFieldTypes(results, expected)
  testHandlesNullData('fetchGetMetric', san.SAN_SOCIAL_DOMINANCE, slug, from, to)
  ...
  it('returns a record per hour', () => {
    const addresses = san.SAN_SOCIAL_DOMINANCE(slug, from, to, '1h')
