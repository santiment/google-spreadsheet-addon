...
describe('SAN_MARKETCAP', () => {
  const expected = {
    date: 'string',
    value: 'number'
  }

  const response = san.SAN_MARKETCAP(slug, from, to)
  const headers = response[0]
  const results = response[1]

  testFieldTypes(results, expected)
  testHandlesNullData('fetchGetMetric', san.SAN_MARKETCAP, slug, from, to)
  ...
  it('returns a record per every day', () => {
    const results = san.SAN_MARKETCAP(slug, from, to)
