...
describe('SAN_TOP_HOLDERS_HELD', () => {
  const expected = {
    date: 'string',
    value: 'number'
  }

  const response = san.SAN_TOP_HOLDERS_HELD(slug, from, to)
  const headers = response[0]
  const addresses = response[1]

  testFieldTypes(addresses, expected)
  testHandlesNullData('fetchGetMetric', san.SAN_TOP_HOLDERS_HELD, slug, from, to)
  ...
  it('returns a record per every day', () => {
    const addresses = san.SAN_TOP_HOLDERS_HELD(slug, from, to)
