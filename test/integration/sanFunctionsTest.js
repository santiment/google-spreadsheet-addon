...
describe('SAN_FUNCTIONS', () => {
  ...
  const response = san.SAN_FUNCTIONS(slug, from, to)
  const headers = response[0]
  const addresses = response[1]

  testFieldTypes(addresses, expected)
  testHandlesNullData('fetchGetMetric', san.SAN_FUNCTIONS, slug, from, to)
  ...
  it('returns a record per every day', () => {
    const addresses = san.SAN_FUNCTIONS(slug, from, to)
