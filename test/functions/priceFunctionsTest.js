...
describe('dailyClosingPrice_', () => {
    ...
    beforeEach(() =>
        sandbox.stub(san, 'assertCanAccessHistoricData_').returns(true))

    it('requests the closing price for the end of the day', () => {
        const beginningOfDaytoEndOfDay = sandbox.stub(
            san,
            'beginningOfDaytoEndOfDay_'
        )
        sandbox
            .stub(san.ApiClient_.prototype, 'fetchDailyClosingPrice')
            .returns([{ closePriceUsd: 100.01 }])
...
describe('priceAbsoluteChange_', () => {
    ...
    it('throws "No data" on empty array response', () => {
        sandbox.stub(san.ApiClient_.prototype, 'fetchOhlc').returns([])
        expect(() => san.priceAbsoluteChange_(slug, from, to)).to.throw(
            'No data'
        )
    })
...
describe('pricePercentChange_', () => {
    ...
    it('throws "No data" on empty array response', () => {
        sandbox.stub(san.ApiClient_.prototype, 'fetchOhlc').returns([])
        expect(() => san.pricePercentChange_(slug, from, to)).to.throw(
            'No data'
        )
    })
